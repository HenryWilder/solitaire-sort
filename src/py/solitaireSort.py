...
class FieldStack:
    ...
    def __init__(self, cards: List[Card] = [], faceUp: int = 0):
        self.__cards = cards
        """
        ...
        """
        assert(self.faceUp <= self.numCards)
 ...
 */
def solitaireSort(data: Card[], rules) -> Card[]:
    // const maxTries: number = 3;
    // for (let i = 0; i < maxTries; ++i) {
    sorted: Card[] | null = play(data.slice());
    // if (sorted !== null) {
    return sorted || data;
    // }
    // }
